    ...
    fn to_tokens(&self, output: &mut TokenStream2) {
        ...
        let method_guards = {
            ...
            if methods.len() > 1 {
                quote! {
                    .guard(
                        spa_server::re_export::guard::Any(spa_server::re_export::guard::#first())
                            #(.or(spa_server::re_export::guard::#others()))*
                    )
                }
            } else {
                quote! {
                    .guard(spa_server::re_export::guard::#first())
                }
            }
        };
        ...
        let stream = quote! {
            ...
            impl spa_server::re_export::dev::HttpServiceFactory for #name {
                fn register(self, __config: &mut spa_server::re_export::dev::AppService) {
                    #ast
                    let __resource = spa_server::re_export::Resource::new(#path)
                        .name(#resource_name)
                        #method_guards
                        #(.guard(spa_server::guard::fn_guard(#guards)))*
                        #(.wrap(#wrappers))*
                        .#resource_type(#name);

                    spa_server::re_export::dev::HttpServiceFactory::register(__resource, __config)
                }
            }
        };
